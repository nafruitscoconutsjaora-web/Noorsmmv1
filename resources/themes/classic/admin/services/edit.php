<div class="max-w-3xl mx-auto space-y-6">
    <div>
        ...
        <h1 class="text-2xl font-bold text-white tracking-tight">Edit Service #<?= $service['id'] ?></h1>
        <p class="text-xs text-slate-400 mt-1">Update rates, limits, description, or upstream provider mapping.</p>
    </div>
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl">
        <form action="/admin/services/<?= $service['id'] ?>" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="category_id" required class="w-full px-3.5 py-2.5 rounded-xl bg-slate-950 border border-slate-750 text-slate-200 text-xs focus:ring-2 focus:ring-rose-500 focus:outline-none transition">
                        <?php foreach ($categories as $cat): ?>
                            <option value="<?= $cat['id'] ?>" <?= $service['category_id'] == $cat['id'] ? 'selected' : '' ?>><?= e($cat['name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            ...
            <div class="p-4 rounded-xl bg-slate-950/70 border border-slate-800 space-y-4">
                <div class="text-xs font-bold text-rose-400 uppercase tracking-wider">Upstream Provider Binding</div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="provider_id" class="w-full px-3 py-2 rounded-xl bg-slate-900 border border-slate-750 text-slate-200 text-xs focus:ring-2 focus:ring-rose-500 focus:outline-none transition">
                            ...
                            <?php foreach ($providers as $prov): ?>
                                <option value="<?= $prov['id'] ?>" <?= $service['provider_id'] == $prov['id'] ? 'selected' : '' ?>><?= e($prov['name']) ?> (<?= e($prov['currency']) ?>)</option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="provider_service_id" value="<?= e($service['provider_service_id'] ?? '') ?>" placeholder="e.g. 1024"
                    ...
                    <div>
                        ...
                        <input type="number" step="0.0001" name="provider_cost" value="<?= (float)($service['provider_cost'] ?? 0) ?>"
                            class="w-full px-3 py-2 rounded-xl bg-slate-900 border border-slate-750 text-slate-200 text-xs focus:ring-2 focus:ring-rose-500 focus:outline-none transition">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="provider_currency" class="w-full px-3 py-2 rounded-xl bg-slate-900 border border-slate-750 text-slate-200 text-xs focus:ring-2 focus:ring-rose-500 focus:outline-none transition">
                            <option value="USD" <?= ($service['provider_currency'] ?? '') === 'USD' ? 'selected' : '' ?>>USD</option>
                            <option value="INR" <?= ($service['provider_currency'] ?? '') === 'INR' ? 'selected' : '' ?>>INR</option>
                            <option value="EUR" <?= ($service['provider_currency'] ?? '') === 'EUR' ? 'selected' : '' ?>>EUR</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" step="0.01" name="rate" value="<?= (float)$service['rate'] ?>" required
                ...
                <div>
                    ...
                    <input type="number" name="min_quantity" value="<?= (int)$service['min_quantity'] ?>" required
                ...
                <div>
                    ...
                    <input type="number" name="max_quantity" value="<?= (int)$service['max_quantity'] ?>" required
                        class="w-full px-3.5 py-2.5 rounded-xl bg-slate-950 border border-slate-750 text-slate-200 font-mono text-xs focus:ring-2 focus:ring-rose-500 focus:outline-none transition">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full px-3.5 py-2.5 rounded-xl bg-slate-950 border border-slate-750 text-slate-200 text-xs focus:ring-2 focus:ring-rose-500 focus:outline-none transition"><?= e($service['description'] ?? '') ?></textarea>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 rounded-xl bg-slate-950/40 border border-slate-800 text-xs text-slate-300">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="refill" value="1" <?= !empty($service['refill']) ? 'checked' : '' ?> class="rounded bg-slate-900 border-slate-700 text-rose-600 focus:ring-0">
                    <span>Refill Support</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="cancel" value="1" <?= !empty($service['cancel']) ? 'checked' : '' ?> class="rounded bg-slate-900 border-slate-700 text-rose-600 focus:ring-0">
                    <span>Cancel Button</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="drip_feed" value="1" <?= !empty($service['drip_feed']) ? 'checked' : '' ?> class="rounded bg-slate-900 border-slate-700 text-rose-600 focus:ring-0">
                    <span>Drip-feed Enabled</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="status" value="active" <?= ($service['status'] ?? '') === 'active' ? 'checked' : '' ?> class="rounded bg-slate-900 border-slate-700 text-rose-600 focus:ring-0">
